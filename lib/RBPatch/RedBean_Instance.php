...
/**
 * RedBean_Instance - adapted from RedBean Facade (iiser@unseen.is)
 *
 * Version Information
...
class RedBean_Instance
{
	...
	{
		return Transaction::transaction( $this->adapter, $callback );
	}
	...
	{
		return ( isset( $this->toolboxes[$key] ) );
	}
	...
	*/
	...
	{
		return MultiLoader::load( $this->redbean, $types, $id );
	}
	...
	{
		return $this->redbean->convertToBeans( $type, $rows, $metamask );
	}
	...
	{
		$beans = $this->redbean->convertToBeans( $type, array( $row ), $metamask );
		$bean  = reset( $beans );
		return $bean;
	}
	...
	{
		return array( $this->redbean, $this->adapter, $this->writer, $this->toolbox );
	}
